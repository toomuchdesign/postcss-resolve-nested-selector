...
test(async (t) => {
  ...
  t.deepEqual(await util.resolveChosenSelector(code, 'd'), ['a b c d']);
});

test(async (t) => {
  const code = `.foo { .bar &, a {}}`;
  t.deepEqual(await util.resolveChosenSelector(code, '.bar &'), ['.bar .foo']);
});

test(async (t) => {
  const code = `.foo { .bar &, a, & + &:hover { b {}}}`;
  t.deepEqual(await util.resolveChosenSelector(code, 'b'), [
    '.bar .foo b',
    '.foo + .foo:hover b',
    '.foo a b',
  ]);
...
test(async (t) => {
  ...
  t.deepEqual(await util.resolveChosenSelector(code, 'c > &'), [
    'c > .bar .foo',
    'c > .foo + .foo:hover',
  ]);
});
