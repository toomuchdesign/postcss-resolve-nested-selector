...
test('postcss-nesting Test Case 3', async (t) => {
  ...
  // @NOTE: postcss-nesting resolves as : ["a", "a a", "a b", "b", "b a", "b b"]
  // t.deepEqual(
  //   await util.postcssNestingResolve(code),
  //   await util.allExpected(code)
  // );
  t.deepEqual(await util.allExpected(code), ['a', 'a a', 'b', 'b b']);
});
...
test('postcss-nesting Test Case 5', async (t) => {
  ...
  t.deepEqual(await util.allExpected(code), ['a', 'a', 'a b', 'a b', 'a c']);
});
